...
import { CloudSun, Droplets, Info, Loader2, Thermometer, Wind, X } from 'lucide-react'
...
const LAYER_META: Record<
  WeatherMapLayerId,
  { label: string; description: string; icon: typeof CloudSun; needsOwm: boolean }
> = {
  none: {
    label: 'Desligado',
...
export type WeatherLayerPlacement = 'planToolbarStack' | 'mapBottomLeft' | 'sidebar'
...
type WeatherLayerMapControlsProps = {
  ...
  setOverlay: (next: WeatherMapOverlayPreferences | ((prev: WeatherMapOverlayPreferences) => WeatherMapOverlayPreferences)) => void
...
export function WeatherLayerMapControls({
  ...
  const radarBusy = overlay.layerId === 'radar' && (radarPhase === 'loading' || radarPhase === 'idle')
  ...
  return (
    <div ref={rootRef} className="pointer-events-auto relative w-12 shrink-0">
      {open ? (
        <div
          id={panelId}
          role="dialog"
          ...
          className={cn(
            'glass-surface absolute animate-fade-in overflow-y-auto rounded-2xl p-3.5 shadow-xl',
            panelPositionClass,
          )}
        >
          <div className="mb-3 flex items-start justify-between gap-2">
            <div>
              <h2 className="text-sm font-semibold tracking-tight text-white">Clima no mapa</h2>
          ...
          <div className="mb-3 grid grid-cols-2 gap-1.5" role="radiogroup" aria-label="Tipo de camada">
            {WEATHER_MAP_LAYER_IDS.map((id) => {
              ...
              return (
                <button
                  ...
                  title={disabledOwm ? 'Configure a chave OpenWeather em Ajustes' : meta.description}
                  onClick={() => setLayer(id)}
                  className={cn(
                    ...
                    disabledOwm && 'cursor-not-allowed opacity-45 hover:border-white/10 hover:bg-white/[0.04]',
                  )}
                >
                  <span className="flex w-full items-center gap-1.5">
                    {showSpinner ? (
                      <Loader2 className="size-3.5 shrink-0 animate-spin text-sky-300" aria-hidden />
                    ) : (
                  ...
                  {disabledOwm ? (
                    <span className="text-[10px] font-normal text-amber-200/80">Requer chave OWM</span>
                  ) : null}
                </button>
              )
            })}
          ...
          <div className={cn('space-y-2', !active && 'pointer-events-none opacity-40')}>
            <div className="flex items-center justify-between gap-2 text-[10px] font-medium uppercase tracking-wide text-[#8a8a8a]">
              <span>Opacidade</span>
              <span className="tabular-nums text-[#c6c6c6]">{Math.round(clampOpacity(overlay.opacity) * 100)}%</span>
            </div>
            <input
              type="range"
              min={22}
          ...
          <div className="mt-3 flex items-start gap-2 rounded-xl border border-white/[0.06] bg-black/25 px-2.5 py-2">
            <Info className="mt-0.5 size-3.5 shrink-0 text-sky-300/90" aria-hidden />
      ...
      >
        <CloudSun className={cn('size-5', active ? 'text-sky-300' : 'text-[#e8e8e8]')} aria-hidden />
        {active ? (
