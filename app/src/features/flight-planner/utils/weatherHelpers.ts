...
export function isWeatherUnavailableCopy(
...
/** Nome curto da força (0–9) alinhado a `windSpeedToBeaufort`. */
const BEAUFORT_LABELS_PT = [
  "Calma",
  "Aragem",
  "Brisa muito leve",
  "Brisa leve",
...
export function beaufortForceLabelPt(bft: number): string {
  const i = Math.max(
    0,
    Math.min(BEAUFORT_LABELS_PT.length - 1, Math.round(bft)),
  );
  return BEAUFORT_LABELS_PT[i];
}
...
export function assessFlightConditions(
  ...
  const precipNow = weather.isPrecipitatingNow ?? weather.rainMmH > 0.08;
  const gusts = weather.windGustsMs ?? weather.windSpeedMs;

  if (weather.windSpeedMs > spec.maxSpeedMs * 0.6) {
    issues.push(
      "Vento acima do recomendado para missão fotogramétrica de precisão.",
    );
  } else if (weather.windSpeedMs > spec.maxSpeedMs * 0.45) {
    warnings.push(
      "Vento moderado: considere reduzir altitude e aumentar sobreposição.",
    );
  }
  ...
  if (gusts > spec.maxSpeedMs * 0.72) {
    ...
    warnings.push(
      "Rajadas moderadas: monitore estabilidade e aborte se oscilar demais.",
    );
  }
  ...
  if (code >= 95) {
    ...
    warnings.push(
      "Precipitação leve (chuvisco); proteja o equipamento e avalie visibilidade.",
    );
  }

  if (weather.cloudCoveragePct > 85) {
    warnings.push(
      "Cobertura de nuvens alta pode reduzir contraste em superfícies homogêneas.",
    );
  }
  ...
  if (next.length > 0) {
    ...
    if (maxProb >= 70 && maxPrecip >= 0.3) {
      warnings.push(
        "Alta probabilidade de chuva nas próximas horas — reveja a janela de voo.",
      );
    } else if (maxProb >= 55) {
      tips.push(
        "Probabilidade de precipitação moderada nas próximas horas; acompanhe o radar.",
      );
    }
  }
  ...
  tips.push(
    `Nível Beaufort estimado: ${windSpeedToBeaufort(weather.windSpeedMs)}.`,
  );
