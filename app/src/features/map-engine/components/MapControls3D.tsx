...
export function MapControls3D({ onBearingReset, onPitchChange, onZoom, visible }: MapControls3DProps) {
  ...
  const btnSize = cn(touchTargetClass(bp), bp === 'tablet' ? 'h-12 w-12' : 'h-9 w-9')
  ...
  return (
    ...
    >
      {/* Reset norte */}
      <ControlButton className={btnSize} onClick={onBearingReset} title="Alinhar ao norte">
      ...
      {/* Aumentar pitch */}
      <ControlButton className={btnSize} onClick={() => onPitchChange(15)} title="Aumentar inclinação (+15°)">
      ...
      {/* Diminuir pitch */}
      <ControlButton className={btnSize} onClick={() => onPitchChange(-15)} title="Diminuir inclinação (-15°)">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" aria-hidden>
      ...
      {/* Zoom in */}
      <ControlButton className={btnSize} onClick={() => onZoom(1)} title="Zoom in">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" aria-hidden>
      ...
      {/* Zoom out */}
      <ControlButton className={btnSize} onClick={() => onZoom(-1)} title="Zoom out">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" aria-hidden>
          <path d="M5 12h14" />
        </svg>
      </ControlButton>
